open Printf
module Scheduler = Sched_work_stealing.Make(struct let num_domains = 4 end)
...
let main () =
  ...
  (* ... *)
  for i = 0 to 99 do
    fork_on (fun () -> run (RLock.acq l) () ;
                       if run (RLock.try_acq l) ()
                       then ignore @@ run (RLock.rel l) ()
                       else assert false ;
                       ignore @@ run (RLock.rel l) () ;
                       run (CDL.count_down cdl) ()) (i mod 4);
    fork_on (fun () -> if run (RLock.try_acq l) () then
                       (lock_and_call l 100 ;
                       ignore (run (RLock.rel l) ())) ;
                       run (CDL.count_down cdl) ()) (i mod 4)
  done ;
